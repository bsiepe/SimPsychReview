...
Prepare the data
TODO:
- some ambiguity in MC Errors, e.g. "mcerrors_q16 in (https://doi.org/10.3758/s13428-021-01580-y): plot, table vs table and figures" 
- how to deal with "?" and "+" and so on
...
sim_res_fac_clean <- sim_res_fac %>% 
  ...
  select(!c(x1, year, issue, journal, quoteaims_q3, ncondsnotes_q6,
            nsimjustifiednotes_q9,
            pmnotes_q15, userwrittenquote_q19, codelink_q20, comments, 
            reproducibilitynote, dateofreview)) %>% 
  ...
  mutate(across(c(nsim_q8, nestimands_q11, pmsclear_q15, mcerrors_q16),
                ~gsub("NA", "unclear", .)))
  # for now: filter out agreement where we looked at different sim study
...
```{r fig.align = "center", fig.height = 8*1.5, fig.width = 5*1.5}
theme_bs <- function(){
  # add google font
