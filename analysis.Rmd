...
Note BS: I changed the font and suggested an alternative color palette in the plot for Q2. Happy to change back if needed.
...
q4 <- sim_res_fac %>% 
    mutate(dgptype_q4 = as.factor(dgptype_q4)) %>% 
...
q8 <- ggplot(data = sim_res_num, aes(x = log(nsim_q8))) +
    ...
    scale_x_continuous(breaks = log(breaks), labels = labs)+
...
q14_plot_max <- max(table(sim_res_fac$nmethods_q14)) + 5
...
q14 <- ggplot(data = sim_res_num, aes(x = nmethods_q14, fill = journal)) +
    geom_bar() +
    scale_x_continuous(breaks = seq(1, 15), limits = c(0.5, 15)) +
...
q15a_new <- sim_res_fac %>% 
    ...
    geom_bar() +
    labs(x = NULL, title ="What is the evaluation target of the simulation?", 
         fill = "Journal", caption = "Double entries combined") +
    scale_fill_manual(values = cols) + 
    scale_y_continuous(limits = c(0, q15_plot_max), expand = c(0,0))+
    theme(panel.grid.major.y = element_blank()) +
    coord_flip()
...
q15b <- sim_res_fac %>%
    ...
    labs(x = NULL, 
         ...
         fill = "Journal",
         caption = "Abs. and rel. Bias counted towards Bias") +
    scale_fill_manual(values = cols) + 
...
q16_plot_max <- max(table(sim_res_fac$mcerrors_q16)) + 5
...
q18b <- sim_res_fac %>% 
  ...
  scale_fill_manual(values = cols) + 
  scale_y_continuous(limits = c(0, 81), expand = c(0,0))+
  theme(panel.grid.major.y = element_blank()) +
  coord_flip()
q18b
...
q22_plot_max <- max(table(sim_res_fac$compenvironment_q22)) + 5
...
```{r fig.height = 7, fig.width = 10.5, warning=FALSE}
...
```{r fig.height = 15, fig.width = 18, warning=FALSE}
## composite plot 2 - descriptives
plotList <- lapply(X = list(q2, q4, q6, q7a, q7b, q8, q14, 
...
# Descriptives
The following still needs some cleaning.
```{r}
...
sim_res_fac %>%
    ...
    gather(key = "PM", value = "count", "Convergence", "Bias", "(R)MSE", 
           "Empirical SE", "Coverage", "Type I error rate", 
           "Power", "CI width", "Other") %>%
    bind_rows(q15_other) %>% group_by(PM) %>% summarize(sum = sum(count)) %>% arrange(desc(sum))
